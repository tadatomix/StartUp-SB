...
namespace StorybrewScripts
{
    class Lyrics : StoryboardObjectGenerator
    {
        ...
        {
            var file = $"{ProjectPath}/assetlibrary/Lyrics/Lyrics";
            AddDependency(file);
        ...
        void JapaneseLyrics(double beat)
        {
            var file = $"{ProjectPath}/assetlibrary/Lyrics/LyricsJap";
            AddDependency(file);

            var font = LoadFont("sb/f/jap", new FontDescription()
            {
                    ...
                    if (!String.IsNullOrEmpty(part[0]) && int.TryParse(part[0], out o))
                    {
                        ...
                        foreach (var letter in text) 
                        {
                            if (letter != '_' && letter != ',' && letter != '-' && letter != '/') 
                            {
                                ...
                                if (!texture.IsEmpty) 
                                {
                                    var position = new Vector2(320 - width / 2 + letterX, PositionY - 7) + texture.OffsetFor(OsbOrigin.Centre) * scale;
